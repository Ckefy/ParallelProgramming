...
class TxVar<T>(initial: T)  {
    ...
    fun openIn(tx: Transaction, update: (T) -> T): T {
        ...
        while (true) {
            ...
            val curValue = curLoc.contValue(tx) { owner -> owner.abort() }
...
private class Loc<T>(
    ...
    fun contValue(trans: Transaction, func: (Transaction) -> Unit): Any? {
        if (trans === owner) {
            return newValue
        }
        if (owner.status == TxStatus.ABORTED) return oldValue
